...
export async function POST(req: Request) {
  ...
  await db.schema
    ...
    .addForeignKeyConstraint('fk_rental_cars', ['car_id'], 'cars', ['id'])
    .addForeignKeyConstraint('fk_rental_users', ['user_id'], 'users', ['id'])
    .execute();
  ...
  const newRental = await db
    ...
    .values({
      ...
      drop_off_time,
      user_id,
      status: payment_method === 'PAYPAL' ? 'pending' : 'complete',
      subtotal,
      tax,
      discount,
      total,
  ...
  await db.schema
    ...
    .addColumn('car_rental_id', 'integer', (cb) => cb.notNull())
    .addColumn('user_id', 'uuid', (cb) => cb.notNull())
    .addColumn('transaction_id', 'varchar(50)')
    .addColumn('status', 'varchar(255)')
    .addColumn('method', 'varchar(100)', (cb) => cb.notNull())
    ...
    .addForeignKeyConstraint('fk_payment_users', ['user_id'], 'users', ['id'])
    .execute();
