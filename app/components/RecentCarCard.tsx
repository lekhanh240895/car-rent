...
import { getCarDetail } from '../lib/actions';
...
async function RecentCarCard({ t, className, vertical, id, locale }: Props) {
  const item = await getCarDetail(t, locale, id);
  const { capacity, steering, gasoline, price, name, type, images } = item;
  const isDiscount = false;

  return (
    <div
      className={clsx(
        'flex flex-shrink-0 flex-col rounded-[10px] bg-white p-4 text-sm dark:bg-black dark:text-white dark:ring-1 dark:ring-white md:h-[388px] xl:p-6',
        className,
