...
function convert() {
  ...
  Object.keys(workbook.Sheets).forEach((sheetName) => {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      const row = rows[i];
      const [fileName, key, vi, en] = transform(sheetName, row, i);

      if (fileName) {
        jsonData[fileName] = jsonData[fileName] || { vi: {}, en: {} };
        jsonData[fileName].vi[key] = vi;
...
function createJsonFiles(output, fileName, jsonData) {
  const { vi, en } = jsonData;
  const viFilePath = path.resolve(output, 'vi', `${fileName}.json`);
  const enFilePath = path.resolve(output, 'en', `${fileName}.json`);
  console.log({ viFilePath, enFilePath });
  fs.writeFile(
    viFilePath,
    `${JSON.stringify(vi, null, TAB_SIZE)}\n`,
    (error) => {
      if (error) {
        throw error;
      }
      /* eslint-disable no-console */
      console.log(`Completed: ${viFilePath}`);
    }
  );
  fs.writeFile(
    enFilePath,
